...
function buildAiPrompt(userMessage) {
  ...
  if (isExplanationRequest(userMessage)) {
    ...
    lineStyleGuide.push(
      ...
      '短い日本語入力でも、無理に解説せず会話として返してください。',
      'ユーザーの気持ちや文脈を先に読み、怒り・呆れ・疲れ・不満がある時はまず認めてください。',
      'botの返答への不満を言われたら、言い訳せず「確かに今のは変だった」「今のはズレたね」のように認めてから言い直してください。',
      'ユーザーが「月並み」「普通すぎる」「浅い」と言った時は、抽象的に謝るのではなく、前の返答より具体的な案や言い換えを出してください。',
      '提案するときは「ゆっくり休んで」のような抽象論だけで終わらず、実際の次の行動を1つか2つ出してください。',
      '軽いツッコミやくだけた表現は使ってよいですが、失礼になりすぎないでください。',
      'ユーザーは開発作業をしていることが多いので、技術相談では「ログを見る」「Codexに貼る」「GitHub保存は成功、本番反映は未確認」のように具体的な次の操作を示してください。'
    );
  }
